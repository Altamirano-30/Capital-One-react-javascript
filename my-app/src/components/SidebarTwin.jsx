import { NavLink } from "react-router-dom";
import "../styles/sidebarTwin.css"; // ← importa tu nuevo CSS

import HomeIcon from "../assets/home2.svg";
import TransferIcon from "../assets/transfer.svg";
import TxnIcon from "../assets/transaction2.svg";
import AccountsIcon from "../assets/accounts2.svg";
import CardsIcon from "../assets/cards2.svg";
import InvestIcon from "../assets/invest2.svg";
import TwinIcon from "../assets/twin2.svg";
import AvatarIcon from "../assets/avatar.svg";

const items = [
  { to: "/", label: "Inicio", icon: HomeIcon },
  { to: "/transfer", label: "Transferencia", icon: TxnIcon },
  { to: "/accounts", label: "Cuentas", icon: AccountsIcon },
  { to: "/cards", label: "Tarjetas", icon: CardsIcon },
  { to: "/invest", label: "Inversiones", icon: InvestIcon },
  { to: "/twin", label: "TwinModel", icon: TwinIcon },
];

export default function SidebarTwin() {
  return (
    <aside className="sidebar sidebar--twin">
      <div className="sidebar__profile">
        <img src={AvatarIcon} alt="Avatar" className="avatar-img" />
        <div className="sidebar__name">Emiliano Altamirano Baez</div>
        <div className="sidebar__rule" />
      </div>

      <nav className="sidebar__nav">
        {items.map((it) => (
          <NavLink
            key={it.to}
            to={it.to}
            end
            className={({ isActive }) =>
              `nav__item ${isActive ? "nav__item--active" : ""}`
            }
          >
            <img src={it.icon} alt="" className="nav__icon-img" />
            <span className="nav__label">{it.label}</span>
          </NavLink>
        ))}
      </nav>
    </aside>
  );
}
